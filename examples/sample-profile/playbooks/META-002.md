...
**Finding:** 1/7 pages fail (missing or wrong length).

**Fix:** Address the finding above so a re-crawl flips META-002 to pass — this is a synthetic worked example, so the fix here is illustrative (a real profile's playbook would name the exact CMS field/template to edit).
